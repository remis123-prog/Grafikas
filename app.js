...
async function fetchWithProxy(targetUrl, proxyFn) {
    const cacheBuster = '_cb=' + Date.now();
    let downloadUrl = proxyFn(targetUrl);
    ...
    if (!response.ok) {
        throw new Error(`Proxy grąžino: ${response.status}`);
    }
    return response;
}
...
async function refreshData() {
    ...
    try {
        ...
        if (!url || url === 'local' || url === 'lokalus') {
            ...
            needsProxy = true;
            downloadUrl = convertCloudUrl(url);
        } else {
        ...
        if (needsProxy) {
            // Bandyti visus proxy iš eilės
            let lastError;
            for (let i = 0; i < CORS_PROXIES.length; i++) {
                try {
                    response = await fetchWithProxy(downloadUrl, CORS_PROXIES[i]);
                    console.log(`✓ Proxy #${i + 1} veikia!`);
                    break; // Pavyko!
                } catch (err) {
                    lastError = err;
                    console.warn(`✗ Proxy #${i + 1} neveikia:`, err.message);
                }
            }
            if (!response) {
                throw new Error('Visi CORS proxy neveikia. Bandykite vėliau arba naudokite "local" režimą. Paskutinė klaida: ' + lastError.message);
            }
        } else {
            // Tiesioginis atsisiuntimas be proxy
            const cacheBuster = '_cb=' + Date.now();
            if (downloadUrl.includes('?')) {
                downloadUrl += '&' + cacheBuster;
            } else {
            ...
            if (!response.ok) {
                if (downloadUrl.startsWith('grafikas_data.json')) {
